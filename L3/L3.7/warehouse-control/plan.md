...
- страница "Профиль" (`/profile`) (требуется доработка):
  ...
  - страница "Вход" (`/auth`) (требуется доработка):
    ...
    - режим "Регистрация":
      ...
      - кнопка переключения режима;
    - Для удобства тестирования должна быть быстрая возможность входа в систему с предустановленными аккаунтами:
      - login: storekeeper, password: password, роль: кладовщик;
      - login: manager, password: password, роль: менеджер;
      - login: auditor, password: password, роль: аудитор.

- страница "Добавление товара" (`/add-item`) (выполнено):
...
- страница "Редактирование товара" (`/edit-item`) (выполнено):
  - загрузка данных с сервера (`GET /items/{id}`);
  - форма для редактирования товара;
  - кнопка "Сохранить" -> `PUT /items/{id}`, переход на страницу "Главная (`/`);
  - кнопка "Отменить" -> переход на страницу "Главная" (`/`);
  
- страница "История изменений" (`/history`) (выполнено):
