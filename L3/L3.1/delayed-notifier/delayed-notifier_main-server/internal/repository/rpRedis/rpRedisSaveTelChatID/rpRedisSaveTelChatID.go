...
func New(parentLg *zlog.Zerolog, rd *pkgRedis.Client) *RpRedisSaveChatID {
	lg := parentLg.With().Str("component", "RpRedisSaveTelChatID").Logger()
...
func (rp *RpRedisSaveChatID) SaveTelegramChatID(ctx context.Context, username string, chatID int64) (err error) {
	...
	lg.Trace().Str("username", username).Int64("chat ID", chatID).Msgf("%s saving name, chat ID to Redis...", pkgConst.OpStart)
	err = rp.rd.Set(ctx, username, chatID, 0)
	if err != nil {
		return pkgErrors.Wrapf(err, "save to Redis, name: %s, chat ID: %d", username, chatID)
	}
	lg.Trace().Str("username", username).Int64("chat ID", chatID).Msgf("%s name, chat ID saved to Redis successfully", pkgConst.OpSuccess)
